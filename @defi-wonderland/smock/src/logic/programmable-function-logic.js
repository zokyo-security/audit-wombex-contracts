...
class ProgrammableFunctionLogic extends watchable_function_logic_1.WatchableFunctionLogic {
    constructor(name, calls$, results$, encoder) {
        ...
        results$.pipe((0, rxjs_1.withLatestFrom)(calls$)).subscribe(async ([result, call]) => {
    ...
    async modifyAnswer(result, call) {
        ...
        if (answer) {
            ...
            if (answer.shouldRevert) {
                result.execResult.exceptionError = new exceptions_1.EvmError('smock revert');
    ...
    getCallAnswer(call) {
        ...
        if (answer)
            return answer;
        answer = (_a = (0, lodash_1.findLast)(this.answerByArgs, (option) => this.isDeepEqual(option.args, call.args))) === null || _a === void 0 ? void 0 : _a.answer;
        if (answer)
            return answer;
